...
function CreatePage(props) {
    ...
    const list = eForm.tasks.map((task, key) => {
      ...
      return (
        <div className='taskElementForCreate' key={key}>
          ...
          <p>
            Дата:{' '}
            {new Date(task.date).toLocaleString('uk-UA', {
              year: 'numeric',
              month: 'numeric',
              day: 'numeric',
            })}
  ...
  const addTask = () => {
    ...
    setTaskParam({ title: '', date: '', period: '', id: uuid() });
  };
  ...
  const addCompany = async () => {
    try {
      ...
      alertShowFunc({
        show: true,
        type: 'success',
        text: 'Компанію збережено',
      });
    } catch (e) {
      alertShowFunc({
        show: true,
        type: 'error',
        text: 'Невдалося додати компанію',
      });
    }
  };
